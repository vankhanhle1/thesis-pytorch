...
class MPCNN(nn.Module):
    ...
    def __init__(self, n_word_dim, n_holistic_filters, n_per_dim_filters, filter_widths, hidden_layer_units,
        ...
        super().__init__()
        self.arch = 'mpcnn'
        self.n_word_dim = n_word_dim  # d0
        self.n_holistic_filters = n_holistic_filters  # number of holistic filters
        self.n_per_dim_filters = n_per_dim_filters  # number of per dim filters
        self.filter_widths = filter_widths
        self.ext_feats = ext_feats  # number of additional features
        ...
        self.final_layers = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Dropout(dropout),
            nn.Linear(hidden_layer_units, 1),
            nn.Sigmoid()
        )
    ...
    def _add_layers(self):
        ...
        for ws in self.filter_widths:
            ...
            per_dim_conv_layers_max.append(nn.Sequential(
                nn.Conv1d(self.in_channels, self.in_channels * self.n_per_dim_filters, ws, padding=padding,
    ...
    def _get_n_feats(self):
        # vertical_holistic, vertical_per_dim, horizontal_holistic
    ...
    def _get_blocks_for_sentence(self, sent):
        block_a = {}  # holistic block
        block_b = {}  # per dim block
        for ws in self.filter_widths:  # ws is 1..n and infinity, n is max window size => number of ws is n+1
            if np.isinf(ws):  # pooling from full representation (no convolution)
                sent_flattened, sent_flattened_size = sent.contiguous().view(sent.size(0), 1, -1), sent.size(
                    1) * sent.size(2)  # sent_flattened_size = height*width of sentence representation
                block_a[ws] = {
            ...
            holistic_conv_out_max = self.holistic_conv_layers_max[ws - 1](sent)  # layers 0..n-1, n is max window size
            ...
            block_a[ws] = {
                ...
                'min': F.max_pool1d(-1 * holistic_conv_out_min, holistic_conv_out_min.size(2)).contiguous().view(-1,
                                                                                                                 self.n_holistic_filters),
                'mean': F.avg_pool1d(holistic_conv_out_mean, holistic_conv_out_mean.size(2)).contiguous().view(-1,
                                                                                                               self.n_holistic_filters)
            }
            ...
            block_b[ws] = {
                'max': F.max_pool1d(per_dim_conv_out_max, per_dim_conv_out_max.size(2)).contiguous().view(-1,
                                                                                                          self.in_channels,
                                                                                                          self.n_per_dim_filters),
                'min': F.max_pool1d(-1 * per_dim_conv_out_min, per_dim_conv_out_min.size(2)).contiguous().view(-1,
                                                                                                               self.in_channels,
                                                                                                               self.n_per_dim_filters)
            }
